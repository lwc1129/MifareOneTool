...
namespace MifareOneTool.Core.Models
{
    public class Utils
    {
    ...
    public class Sector
    {
        ...
        {
            if (uid.Length != 4) { throw new Exception("Invalid 4-byte UID length"); }
        ...
        public string Info(int sec, string sectorLabel = "Sector ", string emptyLabel = " (empty)", string dataLabel = " (data)", string errorLabel = " (error)")
        {
            string info = sectorLabel + sec.ToString();
        ...
        public byte[] KeyB
        {
            ...
            set { for (int i = 10; i < 16; i++) { this._sector[3][i] = value[i - 10]; } }
        }
        public byte[] ACBits
        {
            get { return this._sector[3].Skip(6).Take(4).ToArray(); }
            set { for (int i = 6; i < 10; i++) { this._sector[3][i] = value[i - 6]; } }
        }
    }
    ...
    public class S50
    {
        ...
        {
            if (!File.Exists(file)) { throw new IOException("File not found"); }
            if (new FileInfo(file).Length != 1024) { throw new IOException("S50 dump file size is invalid"); }
        ...
        public void LoadFromMctTxt(string file)
        {
            ...
            if (fileLength < 2200 || fileLength > 2400) { throw new IOException("S50 dump file size is invalid"); }
            ...
            if (lines.Count != 64)
            {
                throw new Exception("File does not contain 64 block lines — may be incomplete or incompatible");
            }
            this.Wipe();
        ...
        public void ExportToMctTxt(string file, string sectorPrefix = "+Sector: ")
        {
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i < 16; i++)
            {
                sb.AppendLine(sectorPrefix + i.ToString());
                for (int j = 0; j < 4; j++)
                {
